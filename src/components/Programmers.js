...
export default function Programmers() {
  ...
  const [allProgrammers, setAllProgrammers] = useState(listOfAwesome)
  const [currentProgrammer, setCurrentProgrammer] = useState()
  // We'll have to use the state hook twice, as we need two slices of state.
  // The programmers list on the one hand, and the id of the featured programmer on the other.

  const getNameOfFeatured = (data) => {
    // Leave this for last!
    ...
    return data[currentProgrammer-1].name
      ...
      <div id='featured' style={style}>
        {
          ...
          currentProgrammer
            ? `🎉 Let's celebrate ${getNameOfFeatured(allProgrammers)}! 🥳`
